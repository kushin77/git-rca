...
def test_concurrent_revocation_safe(app):
    ...
    with app.app_context():
        ...
        tokens = [
            validator.generate_token(f'user_{i}', 'engineer')
            for i in range(10)
        ]
        ...
        threads = [
            threading.Thread(target=revoke_token, args=(token,))
            for token in tokens
        ]
...
def test_user_sessions_tracked(app):
    ...
    with app.app_context():
        ...
        tokens = [
            validator.generate_token('user_123', 'engineer')
            for _ in range(3)
        ]
...
def test_revoke_all_user_sessions(app):
    ...
    with app.app_context():
        ...
        tokens = [
            validator.generate_token('user_xyz', 'engineer')
            for _ in range(5)
        ]
