...
def client():
    ...
    """,
        (
            "inv-001",
            "Test Investigation inv-001",
            "open",
            "high",
            now,
            now,
            "Test investigation for canvas rendering",
            "",
        ),
    )
...
class TestInvestigationCanvasUI:
    ...
    def test_investigations_list_renders_html(self, client):
        ...
        assert (
            b"Investigation Canvas" in response.data
            or b"Investigations" in response.data
        )
    ...
    def test_investigations_list_contains_sample_data(self, client):
        ...
        assert (
            b"Test Investigation" in response.data
            or b"Payment Processing" in response.data
            or b"Database" in response.data
        )
...
class TestInvestigationCanvasAPI:
    ...
    def test_create_investigation_returns_json(self, client):
        ...
        response = client.post(
            "/api/investigations", json={"title": "Test Investigation"}
        )
    ...
    def test_update_investigation_endpoint_exists(self, client):
        ...
        response = client.patch(
            "/api/investigations/inv-001", json={"title": "Updated Title"}
        )
        assert response.status_code == 200
    ...
    def test_add_annotation_endpoint_exists(self, client):
        ...
        response = client.post(
            "/api/investigations/inv-001/annotations",
            json={"text": "Test annotation", "author": "Test User"},
        )
        assert response.status_code in [200, 201]
