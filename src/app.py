...
def create_app(db_path: str = "investigations.db"):
    ...
    def revoke_user_sessions(user_id):
        ...
        try:
            ...
            return (
                jsonify({"error": "Failed to revoke sessions", "details": str(e)}),
                500,
            )
...
def investigations_list():
    ...
    return render_template(
        "investigations_list.html", investigations=investigations_data
    )
...
def update_investigation(investigation_id: str):
    ...
    try:
        investigation = app.investigation_store.update_investigation(
...
def set_email_preferences():
    ...
    try:
        ...
        if digest_frequency not in valid_frequencies:
            return (
                jsonify(
                    {"error": f"digest_frequency must be one of {valid_frequencies}"}
                ),
                400,
            )
...
def send_test_notification():
    ...
    try:
        ...
        if notification_type == "reply":
            ...
            return (
                jsonify({"error": f"Unknown notification_type: {notification_type}"}),
                400,
            )
            ...
            jsonify(
                {
                    "message": (
                        ...
                        if result
                        else "Test notification blocked by preferences"
                    ),
                    "result": result,
                }
            ),
