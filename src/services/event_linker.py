...
class EventLinker:
    ...
    def auto_link_events(
        ...
        try:
            inv_time = datetime.fromisoformat(
                investigation.created_at.replace("Z", "+00:00")
            )
    ...
    def _semantic_match(
        ...
        keywords = [
            word.lower() for word in investigation_title.split() if len(word) > 3
        ]

        # Search in event fields
        event_text = " ".join(
            [str(v).lower() for v in event.values() if isinstance(v, (str, int, float))]
        )
